...
export function JobCard({ job }: { job: JobRow }) {
  const fitColor = job.fit === "적합" ? "bg-emerald-100 text-emerald-800" : job.fit === "조건부" ? "bg-amber-100 text-amber-800" : "bg-zinc-100 text-zinc-600";
  return (
    <li className="rounded-lg border bg-white p-4 transition hover:shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <p className="text-sm text-zinc-600">{job.company} · {job.location || "지역 미상"} · {job.exp_text || (job.exp_min === 0 ? "신입" : job.exp_min ? `경력 ${job.exp_min}년↑` : "경력 미상")}</p>
        </div>
